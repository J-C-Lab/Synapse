...
落到实处，就是一个桌面底座：内置 AI 智能体——具备崩溃可恢复的运行、本地 skill 与可委派的后台智能体——
把插件能力暴露为 **MCP 工具**的沙箱化插件运行时、加密的**局域网设备间**传输，以及一套完整的**插件市场**
（后端 + Web 门户）用于发布、浏览、评分与治理。
...
- **崩溃可恢复的运行**：每一次交互、后台、委派轮次都是一份持久化 checkpoint（冻结的模型/工具配置、有序工具调用账本、预算账本）；中断的运行会在重启后自动续跑或安全放弃，恢复与审计信息在**运行记录**面板中可见
- **本地渐进式 skill**：智能体按需从你的用户/工作区目录发现并激活 `SKILL.md` 包；内容被不可变地捕获并当作不可信内容处理——skill 只能传授工作流，不能扩大该运行的工具或能力范围
- **委派子智能体**：一次性、限定范围的 `spawn_subagent` 用于内联委派；此外还有会话所有权的**异步子任务**（`start`/`check`/`list`/`cancel`），拥有自己的最小权限工具集、有限 token 预算与有界并发
- **事件驱动的后台智能体**：热键、定时器、文件系统监听、轮询类触发器（如 GitHub 收件箱监听）可以在不打开对话窗口的情况下触发一轮智能体执行，受单触发器预算准入与熔断器约束
- **能力治理**：每一次插件动作都是限定范围的，且逐次调用即时审批（无整体自动放行）、可审计、可撤销，并在插件升级间安全迁移
- **凭据代理**：OAuth 设备码/PKCE 流程配合加密保管库与自动令牌刷新，让插件能代你行事而无需智能体或插件本身接触明文密钥
- **受治理的本地执行**：需审批的 shell / 文件 / 补丁工具集在绑定的工作区内运行，具备命令策略、环境变量过滤与可恢复的捕获输出
...
├─ src/                              # Electron 桌面应用
│  ├─ main/                          # 主进程
│  │  ├─ ai/                         # 持久化智能体运行时（checkpoint、skill、子智能体）、provider、MCP 客户端、记忆
│  │  ├─ mcp/                        # Synapse 作为 MCP server
│  │  ├─ plugins/                    # 插件宿主、沙箱、能力治理、凭据代理、触发器
...
│  └─ renderer/                      # React SPA（主页、对话、运行记录、启动器、插件、市场、局域网、设置）
│
├─ packages/
...
pnpm electron:build:win # Windows x64：NSIS + MSI — 官方支持的发布打包命令
# pnpm electron:build:mac / :linux — 不受支持的本地便捷脚本（不会发布）
...
提交前在本地跑完整检查——与 CI 及 Checkpoint R 发布门禁一致：
...
详见 [TESTING.md](./TESTING.md) 中的冒烟清单，以及 [CI_CD.md](./CI_CD.md) 中的 Windows-only 发布配置、dry-run 证明与发布清单。

> CI（GitHub Actions）在每个指向 `main` 的 PR 上运行同样的质量门、测试 + eval 套件，以及未签名的
> Windows x64 Electron 构建（含打包后 renderer + MCP 就绪检查）。公开的 GitHub Release 目前仅发布
> Windows x64 NSIS/MSI — 不含 macOS 或 Linux 产物。
