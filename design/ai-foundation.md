...
export type ToolResult =
  | { content: ToolContentBlock[]; isError?: boolean; structured?: unknown }
export type ToolContentBlock =
  | { type: "text"; text: string }
  | { type: "json"; json: unknown }
  | { type: "image"; path: string; mimeType: string } // 复用沙箱内 plugin-data 路径

export type ToolHandler<I = unknown> = (input: I, ctx: ToolContext) => Promise<ToolResult> | ToolResult
...
1. **会话存储**:SQLite(better-sqlite3,需原生模块) vs 纯 JSON(零原生依赖,符合现有"无 native toolchain")。倾向后者起步。
2. **MCP server 传输**:仅 stdio(给 Claude Code/Desktop) 起步,还是同时上本地 HTTP/SSE?
3. **工具命名**:`<pluginId>/<name>` 是否过长影响模型选择?可加短别名表。
4. **预算/计费**:是否需要会话级 token 预算与用量展示(BYOK 下用户自付,但可视化有用)。
5. **`commands` 是否自动桥接为工具**:默认不自动(语义/返回值是 UI),仅 `tools` 显式声明;是否提供 opt-in 适配器?

> 确认后即可进入 **P0**:落 `contributes.tools` 的清单类型 + JSON Schema + SDK 契约 + CLI 校验(纯类型/校验层,零运行时风险,可立即开工)。
